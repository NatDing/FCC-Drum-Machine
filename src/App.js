...
} else { return (<div className="keyboard">
      {sounds.map((sound) => <KeyboardKey play={play} sound={{...sound, url: "#"}} />)}
      </div>)

 }
}
const Control = ({ power, powerChange, volume, handleVolumeChange, name, changeSounds }) => {
  return <div className="control">
   <button onClick={powerChange}>Power: {power ? "ON" : "OFF"}</button>
   <h2>Volume: {Math.round(volume * 100)}</h2>
   <input max="1" min="0" step="0.01" type="range" value={volume} onChange={handleVolumeChange} />
   <h2 id="display">{name}</h2>
   <button onClick={changeSounds}>Change Kit</button>   
  </div>
}
  ...
  const changeSounds = () => {
    ...
    <div id="drum-machine">
      ...
      <div className="wrapper">
        ...
        <Control powerChange={powerChange} power={power} volume={volume} handleVolumeChange={handleVolumeChange} name={soundName || soundsName[soundType]} changeSounds={changeSounds}/>
      </div>
    </div>
    )};
